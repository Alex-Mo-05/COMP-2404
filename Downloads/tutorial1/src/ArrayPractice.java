...
public class ArrayPractice{
  ...
  public static int[] merge(int[] a, int[] b){
    ...
    int state = 0;
    int newarr[];
    newarr = new int[a.length + b.length];
    for (int i=0; i<newarr.length; i++){
      if (aState < a.length && bState < b.length){
        if (a[aState] <= b[bState]){
          newarr[state] = a[aState];
          state ++;
          aState ++;
        } else{
          newarr[state] = b[bState];
          state ++;
          bState ++;
        }
      } else {
        if (aState < a.length){
          for (int z = aState; z < a.length; z++){
            newarr[state] = a[z];
            state ++;
            aState ++;
          }
